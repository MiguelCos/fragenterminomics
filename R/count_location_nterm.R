...
#it will also generate a count_info object, summarizing the relevant annotation info for counting
...
count_location_nterm <- function(tmt_report_annotated){
    ...
    count_info <- tmt_report_annotated %>% 
    ...
    n_aa_bef_acet <- count_info_acetylated %>%
                        ...
                        mutate(`N-term type` = "Acetylated",
                               ...
                               aa_before = fct_reorder(aa_before, n, .desc = TRUE))
    ...
    plot_aa_before <- ggplot(counts_aa_before,
                        ...
                        #labs(title = "Nr of Acetylated peptides per location")+
    ...
    plot_norm_locat <- ggplot(counts_norm_locat,
                              aes(x = reorder_within(normalized_location, n, `N-term type`), y = n)) + 
                        geom_bar(stat = "identity") +
                        scale_x_reordered() +
                        coord_flip() + 
                        #scale_y_continuous(limits = c(0, 300)) + 
